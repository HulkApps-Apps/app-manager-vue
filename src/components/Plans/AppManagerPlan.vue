  ...
  >
    ...
    <PStack slot="primaryAction">
      <PStackItem style="margin-top: 20px">
        <PButtonGroup class="btn-group" segmented>
          <PButton :style="selectedPlan === 'monthly' ? monthlySelectedStyle : monthlyStyle "
                   @click="selectPlan('monthly')">
            <p style="font-size: 17px; font-weight: 500" slot="default">{{ ('Monthly') }}</p>
          </PButton>
          <PButton :style="selectedPlan === 'annually' ? yearlySelectedStyle : yearlyStyle "
                   @click="selectPlan('annually')" :primary="selectedPlan === 'annually' ">

            <PStack slot="default">
              <PStackItem>
                <PTextContainer spacing="tight" style="margin-top:0">
                  <PHeading>{{ ('Annually') }}</PHeading>
                  <p style="margin-top: -3px; color: #E2C138;font-family: inherit "><b
                      style="font-size: 12px;">{{ ('save') }} 20%</b></p>
                </PTextContainer>
              </PStackItem>
              <PStackItem>
    ...
    <PLayout class="custom-plan">
      <PLayoutSection>
            ...
            <PDataTable class="plan-table">
              <template slot="head">
                <PDataTableRow v-if="selectedPlan === 'monthly'" style="box-shadow: none!important;">
                  ...
                  <template v-for="(plan, key) in monthlyPlan">
                    <PDataTableCol
                        :class="{'first-column': key === 0, 'plan-heading': true, 'last-column': (key+1) === monthlyPlan.length}"
                        :style="activePlanStyle(plan)">
                ...
                <PDataTableRow v-else>
                  ...
                  <template v-for="(plan,key) in yearlyPlan">
                    <PDataTableCol
                        :class="{'first-column': key === 0, 'plan-heading': true, 'last-column': (key+1) === yearlyPlan.length}"
                        :style="activePlanStyle(plan)">
                      <b style="font-size: 16px">{{ (plan.name) }}</b>
                      <p style="display: flex;margin-top: 10px">
                        <PHeading style="font-size: 25px; font-weight: 700;">${{ plan.price.toFixed(2) }}</PHeading>
              ...
              <template slot="body">
                ...
                >
                  ...
                  <PDataTableCol v-for="(plan, cIndex) in selectedPlan === 'monthly' ? monthlyPlan : yearlyPlan"
                                 :key="`cell-${cIndex}-row-${rIndex}`" :style="activePlanStyle(plan)">
                ...
                <PDataTableRow v-if="plans.length" class="row-alignment">
                  ...
                  <PDataTableCol v-for="(plan, cIndex) in selectedPlan === 'monthly' ? monthlyPlan : yearlyPlan"
                                 :key="`cell-${cIndex}-row-plan`" style="max-width: 0">
                    ...
                    <PTextContainer
                        v-if="plan.store_base_plan && shop.plan && plan.shopify_plans.includes(shop.shopify_plan) && !isCurrentPlan(plan)"
                        class="footer-note-container">
                      <PTextStyle class="text-break"
                                  v-if="plan.store_base_plan && !plan.shopify_plans.includes(shop.shopify_plan) && !(isCurrentPlan(plan)) && !isSamePlanInOtherInterval(plan)">
                        Note: On account of your recent Shopify plan upgrade, you should consider upgrading your current
                        app plan
                      </PTextStyle>
                    </PTextContainer>
                  </PDataTableCol>
        ...
        <PStack v-if="onboard && !shop.has_plan" class="choose-plan-btn" alignment="center" distribution="center"
                vertical>
          <PStackItem fill>
...
import PStack from "@/components/polaris-vue/src/components/PStack/PStack";
import PStackItem from "@/components/polaris-vue/src/components/PStack/components/PStackItem/PStackItem";
import {PButton} from "@/components/polaris-vue/src/components/PButton";
import {PButtonGroup} from "@/components/polaris-vue/src/components/PButtonGroup";
import {PHeading} from "@/components/polaris-vue/src/components/PHeading";
import {PLayout} from "@/components/polaris-vue/src/components/PLayout";
import {PLayoutSection} from "@/components/polaris-vue/src/components/PLayout/components/PLayoutSection";
import {PTextContainer} from "@/components/polaris-vue/src/components/PTextContainer";
import {PDataTable} from "@/components/polaris-vue/src/components/PDataTable";
import {PDataTableCol} from "@/components/polaris-vue/src/components/PDataTable/components/PDataTableCol";
import {PDataTableRow} from "@/components/polaris-vue/src/components/PDataTable/components/PDataTableRow";
import {PIcon} from "@/components/polaris-vue/src/components/PIcon";
import {PTextStyle} from "@/components/polaris-vue/src/components/PTextStyle";
